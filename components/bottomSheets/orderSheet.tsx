...
export default function orderSheet({closeOrder,...item}:order) {
    ...
    try {
     const response = await useLocalAuth()
      ...
      if(response?.success){
        ...
        const result = dispatch({type:'CONFIRM_ORDER'})
  ...
  return (
    <SafeAreaView style={[{backgroundColor:Colors.light.background,flex:1},tw`w-full px-2 py-2`]}>
      ...
      {OrderState == 'active'?(<ScrollView style={tw`w-full px-2`}>
        ...
        <View style={[{...styles.IosBoxShadow,...styles.androidBoxShadow},tw`w-full bg-white rounded-2xl flex items-center justify-center mt-2 my-2 px-2`]}>
          ...
          <View style={[{...styles.IosBoxShadow,...styles.androidBoxShadow},tw`w-full bg-white rounded-xl flex flex-row justify-between items-center px-2 py-2 mt-2`]}>
            ...
            <View style={[{...styles.IosBoxShadow,...styles.androidBoxShadow},{backgroundColor:Colors.light.VyreYellow},tw`flex flex-row rounded-2xl`]}>
              <Text style={[{},tw`font-bold text-white text-center mx-2 text-lg`]}>{'80%'}</Text>
            </View>

          </View>

          <View style={tw`w-full bg-transparent my-8 flex flex-row justify-center items-center`}>
            <Text style={tw`font-bold text-center mx-2 text-lg`} darkColor='black'>{'You Send'}</Text>
        ...
        <View style={[,tw`w-full rounded-lg flex flex-row justify-between items-center px-2 py-2`]}>

          <View style={tw`flex flex-row items-center justify-center`}>
          <Ionicons style={[tw`px-1 py-2`]} name="return-down-forward" size={35} color={'black'} />
            <View style={tw`flex items-center justify-center`}>
              <Text style={tw`font-bold text-center mx-2 text-lg`} darkColor='black'>{'You Get'}</Text>
              <Nunito style={tw`font-bold text-gray-500 text-center mx-2 text-sm`}>{' Final Amount'}</Nunito>
            </View>
          </View>

          <View style={[{...styles.IosBoxShadow,...styles.androidBoxShadow},tw`w-1/2 bg-white px-3 py-2 flex flex-row rounded-2xl`]}>
            <View style={[tw`w-full h-10 flex flex-row items-center justify-center rounded-2xl bg-gray-100`]}>
              {/* <Text style={[{},tw`font-extrabold text-center mx-4 my-2 text-xl`]}>{'800'} {item.base}</Text> */}
              <TextInput
                // readOnly
                style={tw`bg-transparent w-3/5 h-full font-bold text-black text-xl`}
                textAlign='center'
                value={Amount}
              />
        ...
        <View style={[,tw`w-full flex flex-row justify-between items-center px-2 py-2`]}>
          <TouchableOpacity onPress={()=>{closeOrder(); setOrderState('active')}} style={[{backgroundColor:'black'},tw`mt-2 w-2/6 px-4 py-2.5 rounded-2xl flex items-center justify-center`]}>
            <Text style={tw`font-bold text-center text-white text-sm`}>{'Cancel'}</Text>
          </TouchableOpacity>

          <TouchableOpacity onPress={()=>{openBottomSheet()}} style={[{backgroundColor:Colors.light.vyreGreen},tw`mt-2 w-3/5 px-4 py-2 rounded-2xl flex items-center justify-center`]}>
      ...
      <BottomSheet ref={sheetRef} animationType='spring' height={'70%'}>
          <AuthBottomSheet action={'CONFIRM_ORDER'} isLoading={isLoading} initAuth={init}/>
      </BottomSheet>
