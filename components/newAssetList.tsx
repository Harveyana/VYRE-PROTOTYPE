...
const newAssetList = ({openSheet}:param) => {
    ...
    <View style={tw`w-full flex flex-row items-center justify-center px-4`}>
      <FlatList
        nestedScrollEnabled
            data={Assets}
            renderItem={({item}) => (
                    <TouchableOpacity onPress={()=>{openSheet()}} style={[{...defaultStyle.IosBoxShadow,...defaultStyle.androidBoxShadow},tw`mx-2 bg-white my-2 py-2 rounded-3xl`]} >
                        <View style={[tw`w-full flex flex-row justify-between bg-transparent rounded-xl px-2.5 py-2`]}>
                            ...
                            <View style={tw`flex flex-row bg-transparent`}>
                                ...
                                <View style={tw`bg-transparent w-1/2`}>
                                    ...
                                    <Text numberOfLines={1} style={[{fontSize:14},tw`font-bold text-gray-500`]}>{item.name}</Text>
                            ...
                            <View style={tw`px-2 ml-2 bg-transparent flex flex-row justify-center items-center`}>
                                ...
                                <TouchableOpacity onPress={()=>{}} style={[{backgroundColor:Colors.light.vyreBlue},tw`rounded-xl`]}>
                                 <Ionicons style={tw`w-6 px-1 py-1 rounded-xl mx-1`} name="md-add-sharp" size={16} color="white" />
                                </TouchableOpacity>
