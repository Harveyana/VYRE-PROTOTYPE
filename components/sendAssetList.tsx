...
import { wallets,wallet,transferTypes } from './data';
import Colors from '../constants/Colors';
import { styles as defaultStyle } from '../constants/Colors';

type param = {
    setAsset:(item:wallet)=>void
}
const AssetList = ({setAsset}:param) => {
    ...
    <View style={tw`w-full pb-4 flex flex-row items-center justify-center px-4`}>
      <FlatList
        nestedScrollEnabled
            data={wallets}
            renderItem={({item}) => (
                    <TouchableOpacity style={[{...defaultStyle.IosBoxShadow,...defaultStyle.androidBoxShadow},tw`w-full bg-white my-2 py-2 rounded-3xl`]} onPress={()=>{setAsset(item)}}>
